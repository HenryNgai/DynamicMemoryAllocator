...
Test(sf_memsuite_student, personalTestCaseOne, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *x = sf_malloc(sizeof(int) * 15);
	...
	cr_assert(hp->info.allocated == 1, "Allocated bit is not set!");
	cr_assert(hp->info.block_size << 4 == 32, "Realloc'ed block size not what was expected!");
	cr_assert(hp->info.requested_size == 8, "Requested size not what was expected!");
...
Test(sf_memsuite_student, personalTestCaseTwo, .init = sf_mem_init, .fini = sf_mem_fini) {
...
Test(sf_memsuite_student, personalTestCaseThree, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	int *x = sf_malloc(10231);

	cr_assert_not_null(x, "x is NULL!");

	*x = 4;

	cr_assert(*x == 4, "sf_malloc failed to give proper space for an 10231!");
...
Test(sf_memsuite_student, personalTestCaseFour, .init = sf_mem_init, .fini = sf_mem_fini) {
...
Test(sf_memsuite_student, personalTestCaseFive, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *x = sf_malloc(100);
	sf_malloc(100);
	sf_malloc(100);
	void *y = sf_realloc(x, sizeof(421));
	...
	// This block will coalesce with the block of size 3968.
	assert_free_block_count(2);
	assert_free_list_count(80,1);
	assert_free_list_count(3712,1);
}
